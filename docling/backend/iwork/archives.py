...
"""Readers for the archives an iWork 2013+ document is built from.

Pages, Numbers and Keynote share their engines, and share the archives those
engines write: ``TSWP`` holds text, ``TST`` tables, ``TSD`` drawables and
``TSK`` annotations. A paragraph in a Pages document and a paragraph on a
Keynote slide are the same ``TSWP.StorageArchive``, laid out the same way, so
they are read once here and the per-app modules are left with nothing but their
own document walk.

Only the message and field numbers are format knowledge; the container layer
lives in :mod:`docling.backend.iwork.iwa`. Apple has never published the
schemas, so every number below was established against real documents.
"""
