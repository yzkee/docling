...
In recent years, Large Language Models(LLMs) have been undergoing rapid iteration and evolution[gpt4o,claude35sonnet,gemini1\_5], progressively diminishing the gap towards Artificial General Intelligence(AGI).  
...
Therefore, in terms of architecture, DeepSeek-V3 still adopts Multi-head Latent Attention(MLA)[dsvii] for efficient inference and DeepSeekMoE[deepseekmoe] for cost-effective training.   
...
Low-precision training has emerged as a promising solution for efficient training[bf16train, fp16train, fp8lm, llm.int8], its evolution being closely tied to advancements in hardware capabilities[fp8format, hifp8format, microscaling].   
...
Lastly, we emphasize again the economical training costs of DeepSeek-V3, summarized in Table[tab:cost], achieved through our optimized co-design of algorithms, frameworks, and hardware.   
...
We first introduce the basic architecture of DeepSeek-V3, featured by Multi-head Latent Attention(MLA)[dsvii] for efficient inference and DeepSeekMoE[deepseekmoe] for economical training.  
...
The basic architecture of DeepSeek-V3 is still within the Transformer[transformer] framework.   
...
Inspired by [meta\_mtp], we investigate and set a Multi-Token Prediction (MTP) objective for DeepSeek-V3, which extends the prediction scope to multiple future tokens at each position.  
...
In Table[tab:dualpipe-bubble], we summarize the pipeline bubbles and memory usage across different PP methods.   
...
In detail, we employ the warp specialization technique[warp-spec] and partition 20 SMs into 10 communication channels.  
...
Inspired by recent advances in low-precision training[fp8lm, llm.int8, 8-bit-numerical], we propose a fine-grained mixed precision framework utilizing the FP8 data format for training DeepSeek-V3.   
...
As depicted in Figure[fig:fp8\_framework], all three GEMMs associated with the Linear operator, namely Fprop (forward pass), Dgrad (activation backward pass), and Wgrad (weight backward pass), are executed in FP8.   
...
Notably, our fine-grained quantization strategy is highly consistent with the idea of microscaling formats[rouhani2023microscaling], while the Tensor Cores of NVIDIA next-generation GPUs (Blackwell series) have announced the support for microscaling formats with smaller quantization granularity[nvidia\_tensor\_cores].   
...
In order to address this issue, we adopt the strategy of promotion to CUDA Cores for higher precision[Thakkar\_CUTLASS\_2023].   
...
Based on our experiments, setting $N\_C=128$ elements, equivalent to 4 WGMMAs, represents the minimal accumulation interval that can significantly improve precision without introducing substantial overhead.

Mantissa over Exponents.   
In contrast to the hybrid FP8 format adopted by prior work [transformerengine, fp8lm, hfp8], which uses E4M3 (4-bit exponent and 3-bit mantissa) in Fprop and E5M2 (5-bit exponent and 2-bit mantissa) in Dgrad and Wgrad, we adopt the E4M3 format on all tensors for higher precision.   
...
We aspire to see future vendors developing hardware that offloads these communication tasks from the valuable computation unit SM, serving as a GPU co-processor or a network co-processor like NVIDIA SHARP[nvsharp].   
...
In the training process of DeepSeekCoder-V2[dscodervii], we observe that the Fill-in-Middle(FIM) strategy does not compromise the next-token prediction capability while enabling the model to accurately predict middle text based on contextual cues.   
...
The tokenizer for DeepSeek-V3 employs Byte-level BPE[shibata1999byte] with an extended vocabulary of 128K tokens.   
...
We adopt a similar approach to DeepSeek-V2[dsvii] to enable long context capabilities in DeepSeek-V3.   
...
In Table[tab:main], we compare the base model of DeepSeek-V3 with the state-of-the-art open-source base models, including DeepSeek-V2-Base[dsvii] (our previous release), Qwen2.5 72B Base[qwen2\_5], and LLaMA-3.1 405B Base[llama3\_1\_405b].  
...
In Table[tab:ablation\_nextn], we show the ablation results for the MTP strategy.   
...
In Table[tab:ablation\_noaux\_tc], we show the ablation results for the auxiliary-loss-free balancing strategy.  
...
As illustrated in Figure[fig:expert\_load], we observe that the auxiliary-loss-free model demonstrates greater expert specialization patterns as expected.
...
For the second challenge, we also design and implement an efficient inference framework with redundant expert deployment, as described in Section [sec:inference\_deployment], to overcome it.
...
Similar to DeepSeek-V2[dsvii], we adopt Group Relative Policy Optimization(GRPO)[deepseekmath], which foregoes the critic model that is typically with the same size as the policy model, and estimates the baseline from group scores instead.   
...
Table[tab:chat] presents the evaluation results, showcasing that DeepSeek-V3 stands as the best-performing open-source model.   
...
In addition to standard benchmarks, we also evaluate our models on open-ended generation tasks using LLMs as judges, with the results shown in Table[tab:open].   
...
Table[tab:distill] demonstrates the effectiveness of the distillation data, showing significant improvements in both LiveCodeBench and MATH-500 benchmarks.   
...
During the development of DeepSeek-V3, for these broader contexts, we employ the constitutional AI approach[bai2022constitutional], leveraging the voting evaluation results of DeepSeek-V3 itself as a feedback source.   
